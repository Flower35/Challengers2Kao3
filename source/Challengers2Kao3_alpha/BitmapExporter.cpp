...
bool CBitmapExporter::wykonajZadanie(int jakie)
{
	...
	try
	{
		if (0 == jakie)
		{
			...
			// * Pomiń liczbę obiektów
			przesunPlik(0x04);

			// * Sprawdź, czy na początku jest gadżet "eBitmap"
			czytajPlik(&x, 4);
			czytajPlik(&y, 4);
			if ((0 != x) || (0x1001 != y))
			{
...
////////////////////////////////////////////////////////////////
// Czy bitmapa wymaga alokowania pamięci na Paletę?
// "kao2.00470F40"
////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////
// Zwróć pełną nazwę rodzaju eBitmapy
////////////////////////////////////////////////////////////////
...
bool CBitmapExporter::przygotujTeksture()
{
	...
	/* Odczytaj Szerokość i Wysokość Bitmapy */
	czytajPlik(&x, 0x04);
	czytajPlik(&y, 0x04);
	...
	/* Sprawdź, czy rodzaj jest obsługiwany */
	switch (Rodzaj)
	{
		case DXT1:
		case RGBA8:
	...
	/* Odwróć kolory palety, jeśli istnieje. Odwróć RGBA->BGRA */
	if (isUsingPalette())
	{
	...
	/* Serializuj nazwę pliku z Bitmapą */
	czytajPlik(&x, 0x04);
	if (0 == x)
	{
		czytajPlik(&x, 0x04);
...
////////////////////////////////////////////////////////////////
// Przygotuj końcową nazwę pliku
////////////////////////////////////////////////////////////////
bool CBitmapExporter::przygotujNazwe()
{
	int x;

	/* Ustal format pliku wyjściowego */
	switch (Rodzaj)
	{
		case RGBA8:
		case PAL8_RGBA8:
	...
	/* Sprawdź, czy rzeczywiste formaty są zgodne */
	...
	else
	{
		...
		/* Pamiętaj o ".." na początku plików w Challengers! */
		...
		/* Specjalne sprawdzanie, czy są to pliki z "menu/interfejs3" */
...
void CBitmapExporter::eksportujTeksture()
{
	...
	/* Pomiń plik, jeżeli już istnieje :) */
	...
	/*** Przygotuj odpowiedni nagłówek pliku ***/
	...
	/*** Przygotuj odpowiedni blok wyjściowy (zapamiętaj ilość bajtów) ***/
	...
	if (0x4E == Rodzaj)
	{
		for (int Y=0; Y<(SizeY/2); Y+=2)
		{
			for (int X=0; X<SizeX; X+=4)
			{
				...
				// WYJŚCIE na 32 bajty (32+32+32+32 bitów)
				// X rośnie co 4, bo kolejny output jest co 4 piksele po 32 bity
				// Y rośnie co 4, bo bloki output są 4x4 piksele.
				// stride *4 bo to są Bajty scanline 
	...
	/*** [TGA, 8] 8-bitowa paleta RGBA na pełną TARGA 32-bit ***/
	...
	/*** [BMP, 8] 8-bitowa paleta na 256-kolorową Bitmapę ***/
	else if (BmpExt && (PAL8_RGBA8 == Rodzaj))
	/* (BMP 0x02) */
	{
		for (int Y=0; Y<SizeY; Y++)
	...
	/*** Możesz teraz wyczyścić "piksele1" ***/
	if (nullptr != Piksele1)
	{
		delete[](Piksele1);
		Piksele1 = nullptr;
	...
	/*** Zapisz nagłówek obrazka ***/
	zapiszPlik(header, BmpExt ? 0x36 : 0x12);

	/*** Zapisz paletę, jeżeli jest to bitmapa 256-kolorowa ***/
	if (BmpExt && isUsingPalette())
	{
		zapiszPlik(Paleta, 0x0400);
	}

	/*** Zapisz odpowiednią ilosć bajtów z kolorami ***/
	zapiszPlik(Piksele2, y);

	/*** Opcjonalnie: Zamknij plik i wyczyść ostatnie piksele z pamięci ***/
	CloseHandle(Plik2);
