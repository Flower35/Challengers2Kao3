...
////////////////////////////////////////////////////////////////
// Generuje początek komunikatu o błędzie
////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////
// [OBIEKT_PAK] Sprawdzanie nagłówków podczas exportu
////////////////////////////////////////////////////////////////
...
int CPakExporter::eksportujItem(int rozmiar, eUnicodeString &nazwa)
{
	...
	/* ZAWSZE: Odczytywanie mogło skończyć się na wcześniejszym offsecie */
	/* (plik już istnieje; ostatnie 4 bajty eBitmap; śmieciowe dane VAGp) */
...
int CPakExporter::skanujBlokItems(int rozmiar, int &id)
{
	...
	while ((x = przesunPlik(0)) < rozmiar)
	{
		...
		if ((*(int32_t*)"item") != x)
		{
			...
			/* Jeżeli istnieją języki, to sektor TATE kończy się na wcześniejszym ID! */
			else return 0;
		}
		else
		{
			...
			/* Usuń spacje na końcu nazwy (np. "spoon.def ") */
			for (y = (0x70 - 1); y >= 0; y--)
			{
				if (0x20 == item_name[y])
				{
			...
			/* Przesuń licznik identyfikatorów */
			id++;
		}
	}
	return 0;
...
int CPakExporter::beginExtracting()
{
	...
	try
	{
		/*** Przygotuj i sprawdź archiwum przed eksportem ***/
		if (0 != odczytajNaglowki())
		{
			return 1;
		}

		/*** Rozpocznij wypakowywanie plików z głównego sektora ***/
		...
		/*** Kontynuuj wypakowywanie z sektorów językowych ***/
		for (int i = 0; i < Langs; i++)
		{
